...
$LAMBDA_FUNCTIONS = @(
    @{ Name = "authorizer"; LogicalId = "AuthorizerFunction" },
    @{ Name = "auth"; LogicalId = "AuthFunction" },
    @{ Name = "projects"; LogicalId = "ProjectsFunction" },
    @{ Name = "leads"; LogicalId = "LeadsFunction" },
    @{ Name = "tasks"; LogicalId = "TasksFunction" },
    @{ Name = "budget"; LogicalId = "BudgetFunction" },
    @{ Name = "documents"; LogicalId = "DocumentsFunction" },
    @{ Name = "file-upload"; LogicalId = "FileUploadFunction" }
)
...
foreach ($func in $LAMBDA_FUNCTIONS) {
    ...
    try {
        ...
        # Create zip excluding git files and aws-sdk
        $filesToZip = Get-ChildItem -Path . -Recurse -Exclude @("*.git*") |
